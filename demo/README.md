...
先前的同义改写失败通过受限 Citation selector 修复，题目和预期来源未改；启用模型后，模型可以在
命中的候选证据上进行受控整理，但不能凭空补充来源。Agent 的证据终止约束、增量主题扩展、
`local_only` 模型授权和飞书回复桥接由仓库测试覆盖，因为这些功能不应把公司内部资料写进公开 Demo。
前端框架题仍引用了包含正确 Vue 事实的替代文档，因此来源召回按固定 expected source 记为 96.2%。
