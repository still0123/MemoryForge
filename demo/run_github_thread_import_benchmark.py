#!/usr/bin/env python3
"""Run the frozen GitHub thread-import development cases twice."""
...
DEVELOPMENT = REPO_ROOT / "demo/evaluation/github_thread_import_development.json"
CONFIRMATION = REPO_ROOT / "demo/evaluation/github_thread_import_confirmation.json"
DEVELOPMENT_SHA256 = "b4a8f3269b6c99b9a7d5c3a122621f9186c966bad9170501c49749ce2de0f4ff"
CONFIRMATION_SHA256 = "45de8d774b8d02b0112571ceb8c3b4d43db589cb0a68dbb96579aa2327ad24b0"
TEST_SHA256 = "70be6edefcb388a05bb59a6f604c17722d6b335ff5b75d7e54aad912d4d0228e"
...
def main(argv: list[str] | None = None) -> None:
    ...
    _validate_artifact(REPO_ROOT / "tests/test_github_thread_import.py", TEST_SHA256)
    ...
    print(f"Wrote GitHub thread-import evidence to {output}")
    if not evidence["passed"]:
        raise SystemExit("GitHub thread-import benchmark failed")
...
def _validate_development(suite: dict[str, Any]) -> None:
    if (
        ...
        or suite.get("suite_id") != "github-thread-import-lifecycle"
        or suite.get("suite_revision") != 1
        or suite.get("split") != "development"
        or suite.get("test_file") != "tests/test_github_thread_import.py"
        ...
        raise ValueError("invalid GitHub thread-import development suite")
    cases = suite.get("cases")
    if not isinstance(cases, list) or len(cases) != 5:
        raise ValueError("GitHub thread-import development suite requires five cases")
    ...
    for case in cases:
        if (
            ...
            or not case["pytest_node"].startswith("tests/test_github_thread_import.py::test_")
            or case["expected_status"] != "passed"
        ):
            raise ValueError("invalid GitHub thread-import development case")
...
def _validate_confirmation(suite: dict[str, Any]) -> None:
    if (
        ...
        raise ValueError("invalid GitHub thread-import confirmation suite")
    cases = suite.get("cases")
    if not isinstance(cases, list) or len(cases) != 3:
        raise ValueError("GitHub thread-import confirmation suite requires three cases")
    _validate_case_ids(cases)
    for case in cases:
        if set(case) != {"id", "category", "scenario", "expected"} or any(
            ...
            raise ValueError("invalid GitHub thread-import confirmation case")
...
def _validate_case_ids(cases: list[dict[str, Any]]) -> None:
    ...
    if any(not isinstance(case_id, str) or not case_id for case_id in ids) or len(ids) != len(
        ...
        raise ValueError("GitHub thread-import case IDs must be unique")
