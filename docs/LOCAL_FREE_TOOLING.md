...
```bash
./scripts/check_local.sh local-evidence/v0.4.0

python scripts/build_release.py \
  --output local-evidence/v0.4.0-release
```
...
```bash
git tag -a v0.4.0 -m "MemoryForge v0.4.0"
...
gh release upload v0.4.0 \
  ...
  local-evidence/v0.4.0-release/reproducibility-* \
  local-evidence/v0.4.0-release/SHA256SUMS
```
